...
//! (`docs/public-api/zenquant.txt`) on every `cargo test` run, so API
...
/// NOTE: no `-p zenquant` — the dev-dependency `zengif` pulls a *published*
/// zenquant into the graph, which makes the bare name spec ambiguous, and
/// cargo-public-api 0.52 does not accept `name@version` pkgid specs. The
/// root manifest here IS the zenquant package, so package-less invocation
/// (anchored by `current_dir`) selects it unambiguously.
fn surface(feature_args: &[&str]) -> Vec<String> {
    let mut args = vec!["public-api", "--simplified"];
...
/// gates, sorted for determinism. (`--no-deps` metadata lists only the
/// local workspace package, so the registry zenquant never shadows it.)
