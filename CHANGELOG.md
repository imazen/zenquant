...
- `build_palette_with_stop`, `build_palette_rgba_with_stop`,
  ...
  `quantize_with_stop`/`quantize_rgba_with_stop`. A cancelled call returns
  `QuantizeError::Cancelled` rather than a quietly degraded palette or image.
  Purely additive; the existing no-token functions delegate with
  `enough::Unstoppable`.
- Versioned public-API surface snapshot at `docs/public-api/zenquant.txt`,
  regenerated by the standalone `apidoc/` package (`just api-doc`;
  `just api-doc-check` runs it with `ZEN_API_DOC=check` to verify the committed
  snapshot is current). Note: `apidoc/` has its own manifest and **no CI job runs
  it**, so the snapshot does not self-verify on push — regenerate it in the same
